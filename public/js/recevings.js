    ...
    app.controller("SearchItemCtrl", [ '$scope', '$http', function($scope, $http) {
        ...
        $scope.addReceivingTemp = function(item,newreceivingtemp) {
         ...
         $scope.addCustomItem = function(customitem) {
            ...
            $http.post(site_url+'/item/customcreate', { item_name: customitem.item_name, cost_price: customitem.cost_price, selling_price: customitem.selling_price, quantity: customitem.quantity, type: 'receiving' }).
            success(function(data, status, headers, config) {
                $scope.customitem = { };
