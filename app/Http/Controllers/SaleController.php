...
{ 
    ...
    public function __construct(PaymentType $paymentType, Customer $customer, Sale $sale, Item $item, SaleItem $saleItem)
    {
    ...
    public function refund(Request $request, $id)
    {
        ...
        // $data['invoice'] = $saleObj->getInvoiceNo(['name'=>'SALE', 'id'=>$sale->id]);
        $data['action'] = 'refund';
        if ($request->isMethod('POST')) {
            $input = $request->all();
            $this->validator($input)->validate();
        ...
        return view('sale.index', $data);
            // ->with('sale', $sales)
            // ->with('formurl', '/sales/refund/'.$sale->id)
            // ->with('customers', $customers);
    }
    ...
    public function mailInvoice(Sale $sale){
        ...
        // $data = view('sale.print_invoice', $data)->render();
